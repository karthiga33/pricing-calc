...
CORRECTIONS_TABLE_NAME = "extraction_corrections"
...
CRITICAL RULE — A1 (Cr/Dr / NEGATIVE INDICATOR):
- Some documents show a "Cr" (Credit) or "Dr" (Debit) label next to each row's amount (e.g. "31,242.04 Cr", "21,005.09 Dr").
- Some documents show a negative sign "-" next to or after an amount (e.g. "122,413.20-", "104.00-", "-61.00").
- For each invoice row, capture the indicator into the "A1" field:
  * If the amount has a "Cr" label -> A1 = "Cr"
  * If the amount has a "Dr" label -> A1 = "Dr"
  * If the amount has a negative sign (minus) -> A1 = "-"
  * If none of these -> A1 = "" (empty string)
- Do NOT modify, recalculate, or change the sign of the amount because of the indicator. Keep the amount value exactly as shown (as a positive number in INVOICE_AMOUNT/AMOUNT). The indicator ONLY goes into A1.
- This same A1 logic applies at every level — each invoice/transaction row.

CRITICAL RULE — A2 (CASH DISCOUNT INDICATOR):
- If the document mentions Cash Discount ANYWHERE — written as "CD", "C.D", "C.D.", "Cash Discount", "Cashdiscount" (case-insensitive) — then set A2 = "HEADER DISCOUNT" for EVERY invoice row in the document.
- If the document does NOT mention any cash discount, set A2 = "" (empty string) for all rows.
- The cash discount amount itself still goes into the normal CASH_DISCOUNT / amount fields as before — A2 is only the "HEADER DISCOUNT" text flag.

CRITICAL RULE — SAME DOCUMENT NUMBER WITH POSITIVE AND NEGATIVE ENTRIES (NETTING):
- If the SAME DOCUMENT_NUMBER appears MULTIPLE times with both positive and negative amounts, DO NOT output multiple rows.
- Instead, NET (add) all the amounts for that document number together into ONE consolidated amount, and output ONLY ONE row with that single document number.
- Example: Doc 3008120006709 has +50000 and -70000 -> net = -20000 -> output ONE row with DOCUMENT_NUMBER=3008120006709.
- Store the amount as a POSITIVE number (absolute value) in INVOICE_AMOUNT/AMOUNT. Do NOT store a negative number.
- If the netted (final) value is NEGATIVE, set A1 = "-" for that consolidated row.
- If the netted (final) value is positive, keep A1 as it would normally be (Cr/Dr/empty).
- This netting applies per document number — only merge rows that share the exact same DOCUMENT_NUMBER.

CRITICAL RULE — TDS COLUMN POSITION (GROSS vs NET):
- Look at WHERE the transaction/main amount column sits relative to the TDS column in the table:
  * If the transaction amount column comes BEFORE (to the LEFT of) the TDS column -> that transaction amount is the GROSS amount. Then NET = GROSS - TDS. (compute NET)
  * If the transaction amount column comes AFTER (to the RIGHT of) the TDS column -> that transaction amount is the NET amount. Then GROSS = NET + TDS. (compute GROSS)
- Always output: INVOICE_AMOUNT = GROSS, AMOUNT = NET, TDS_AMOUNT = the TDS value.
- Whichever of gross/net is not directly shown, calculate it using the TDS.

CRITICAL RULE — AMOUNT FIELDS (STRICT DEFINITIONS):
- INVOICE_AMOUNT = the GROSS amount (before TDS deduction).
...
{correction_examples}"""
    ...
    # ── POST-PROCESSING: Net same-document +/- rows into one (code-level) ────
    claude_rows = _net_same_document_rows(claude_rows)

    # ── POST-PROCESSING: Adient India MM/DD/YYYY -> DD/MM/YYYY date fix ──────
    claude_rows = _fix_adient_date_format(claude_rows)

    logger.info("Total rows from Claude (after dedup + validation): %d", len(claude_rows))
    return extracted_data, claude_rows


def _net_same_document_rows(claude_rows):
    """
    If the SAME DOCUMENT_NUMBER appears multiple times with both positive and
    ...
    - Rows with unique document numbers are left untouched.
    """
    ...
    for row in claude_rows:
        ...
        if not doc:
            # No doc number — keep as-is under a unique key
            key = f"__nodoc_{len(order)}"
            groups[key] = [row]
            order.append(key)
            continue
    ...
    for key in order:
        ...
        # Multiple rows with same doc number — check for +/- mix
        amounts = [_to_num(r.get("AMOUNT")) if r.get("AMOUNT") is not None
                   else _to_num(r.get("INVOICE_AMOUNT")) for r in rows]
        # Consider the sign from A1 == "-" as negative too
        signed = []
        ...
        if has_pos and has_neg:
            ...
            if "HEADER DISCOUNT" in existing_a2.upper():
                ...
                base["A2"] = "summed"
            logger.info("Netting: doc %s consolidated %d rows -> %s (A1=%s, A2=%s)",
                        key, len(rows), abs_net, base["A1"], base["A2"])
            result.append(base)
        else:
            # Same sign — keep all rows as-is
            result.extend(rows)

    return result


def _fix_adient_date_format(claude_rows):
    """
    For Adient India Private Limited documents, dates come as MM/DD/YYYY.
    Convert them to DD/MM/YYYY. Detection is by CUSTOMER_NAME containing 'ADIENT'.
    """
    if not claude_rows:
        return claude_rows
    ...
    def _is_adient(row):
        h = row.get("_header", {}) if isinstance(row, dict) else {}
        name = str(h.get("CUSTOMER_NAME") or row.get("CUSTOMER_NAME") or "").upper()
        return "ADIENT" in name

    def _swap_mm_dd(date_str):
        if not date_str:
            return date_str
        s = str(date_str).strip()
        # Match DD/MM/YYYY or MM/DD/YYYY with / or - separators
        m = re.match(r"^(\d{1,2})[/\-.](\d{1,2})[/\-.](\d{2,4})$", s)
        if not m:
            return date_str
        first, second, year = m.group(1), m.group(2), m.group(3)
        # Source is MM/DD/YYYY -> swap to DD/MM/YYYY
        # first = month, second = day  -> output day/month/year
        return f"{int(second):02d}/{int(first):02d}/{year}"

    for row in claude_rows:
        if _is_adient(row):
            if row.get("DOCUMENT_DATE"):
                new_date = _swap_mm_dd(row.get("DOCUMENT_DATE"))
                if new_date != row.get("DOCUMENT_DATE"):
                    logger.info("Adient date fix: %s -> %s", row.get("DOCUMENT_DATE"), new_date)
                    row["DOCUMENT_DATE"] = new_date
...
def _fix_invoice_numbers_from_tables(claude_rows, tables):
    ...
    for table in tables:
        ...
        thirteen_col = _find_13_digit_invoice_column(data_rows, len(header_row))
        if thirteen_col is None:
            continue
...
def build_rows_from_table(table):
    ...
    for idx, name in enumerate(header_row):
        ...
        if doc_num_idx is None and ("your document" in n or "invoice no" in n or "invoice number" in n
                or "inv no" in n or "inv. no" in n or "bill no" in n or "bill number" in n
    ...
    # (format like 3010010344917, 3045010042230, etc.)
    # If found, override doc_num_idx with that column.
    thirteen_digit_col = _find_13_digit_invoice_column(data_rows, len(header_row))
...
def _find_13_digit_invoice_column(data_rows, num_cols):
    """
