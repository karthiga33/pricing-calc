# ABOUTME: Lambda function that monitors user token quotas, BLOCKS users who exceed limits,
# ABOUTME: and sends SNS alerts including "Token Limit Reached - BLOCKED" notifications.
# ABOUTME: Queries CloudWatch PromQL API for usage data, writes to DynamoDB, checks thresholds.
# ABOUTME: Writes BLOCK_STATUS to DynamoDB so the proxy rejects requests for blocked users.
...
def fetch_usage_from_promql():
    ...
    # Try user.name first (IAM Identity Center), then user.email, then fallback to no-user-dimension
    user_dimension = None
    for dim in ("user.name", "user.email", "user.id"):
        test_results = _promql_query(
            f'sum by ("{dim}")(increase({{"claude_code.token.usage"}}[{window}s]))'
        )
        if test_results:
            user_dimension = dim
            break
    ...
    if user_dimension:
        ...
        type_results = _promql_query(
            f'sum by ("{user_dimension}", type)(increase({{"claude_code.token.usage"}}[{window}s]))'
        )
    else:
        # No user dimension — aggregate all usage under a default user
        print("[WARN] No user dimension found (user.name/user.email/user.id). Using total aggregation.")
        results = _promql_query(
            f'sum(increase({{"claude_code.token.usage"}}[{window}s]))'
        )
        type_results = _promql_query(
            f'sum by (type)(increase({{"claude_code.token.usage"}}[{window}s]))'
        )
    ...
    if user_dimension:
        for r in results:
            user = r["metric"].get(user_dimension, "")
            val = float(r["value"][1])
            if user and val > 0:
                users[user] = {"total_tokens": val}
        ...
        # No user dimension — use DEFAULT_USER as placeholder
        default_user = os.environ.get("DEFAULT_USER", "default-user@company.com")
        total_val = 0
        if results:
            total_val = float(results[0]["value"][1]) if results[0].get("value") else 0
        if total_val > 0:
            users[default_user] = {"total_tokens": total_val}
            for r in type_results:
                token_type = r["metric"].get("type", "")
                val = float(r["value"][1])
                if val > 0:
                    if token_type == "input":
                        users[default_user]["input_tokens"] = val
                    elif token_type == "output":
                        users[default_user]["output_tokens"] = val
                    elif token_type in ("cache_read", "cacheRead"):
                        users[default_user]["cache_tokens"] = val
...
def update_quota_metrics(usage_data):
    """Atomically increment UserQuotaMetrics with delta from PromQL."""
...
# ═══════════════════════════════════════════════════════════════════════════════
# ENFORCEMENT: Block user + Send "Token Limit Reached" SNS
# ═══════════════════════════════════════════════════════════════════════════════

def block_user(email, policy, total_tokens):
    """Write BLOCK_STATUS to DynamoDB. Proxy reads this to deny requests."""
    now = datetime.now(timezone.utc)
    current_month = now.strftime("%Y-%m")
    ...
    try:
        quota_table.put_item(Item={
            ...
            "month": current_month,
            "usage_at_block": Decimal(str(int(total_tokens))),
            "limit": Decimal(str(monthly_limit)),
            "policy_type": policy.get("policy_type", "default"),
            "policy_identifier": policy.get("identifier", "environment"),
            "ttl": int((now.replace(day=28) + __import__("datetime").timedelta(days=32)).replace(day=1).timestamp()),
        })
        print(f"BLOCKED user {email} — usage {int(total_tokens):,} exceeds limit {monthly_limit:,}")
    except Exception as e:
        print(f"Error blocking user {email}: {e}")


def unblock_user(email):
    """Remove BLOCK_STATUS if user is under limit (new month / admin reset)."""
    try:
        response = quota_table.get_item(Key={"pk": f"USER#{email}", "sk": "BLOCK_STATUS"})
        if response.get("Item"):
            quota_table.delete_item(Key={"pk": f"USER#{email}", "sk": "BLOCK_STATUS"})
            print(f"UNBLOCKED user {email}")
    except Exception as e:
        print(f"Error unblocking user {email}: {e}")


def send_block_notification(email, policy, total_tokens, month_name):
    """Send SNS: TOKEN LIMIT REACHED — USER BLOCKED."""
    if not SNS_TOPIC_ARN:
        print("SNS_TOPIC_ARN not configured")
        return
    monthly_limit = policy["monthly_token_limit"]
    pct = (total_tokens / monthly_limit) * 100 if monthly_limit > 0 else 0
    subject = f"Claude Code BLOCKED - Token Limit Reached - {email}"
    message = (
        f"TOKEN LIMIT REACHED - USER BLOCKED\n"
        f"{'=' * 50}\n\n"
        f"USER: {email}\n"
        f"MONTH: {month_name}\n"
        f"USAGE: {int(total_tokens):,} tokens\n"
        f"LIMIT: {monthly_limit:,} tokens\n"
        f"PERCENTAGE: {pct:.1f}%\n"
        f"POLICY: {policy.get('policy_type', 'default')}:{policy.get('identifier', 'environment')}\n\n"
        f"ACTION: User access to Claude model is BLOCKED.\n"
        f"They will receive 'Token limit reached' error until next billing period.\n\n"
        f"To manually unblock, delete DynamoDB item:\n"
        f"  Table: {QUOTA_TABLE}\n"
        f"  pk=USER#{email}, sk=BLOCK_STATUS\n"
    )
    try:
        sns_client.publish(TopicArn=SNS_TOPIC_ARN, Subject=subject[:100], Message=message)
        print(f"Sent BLOCK SNS notification for {email}")
    except Exception as e:
        print(f"Error sending block notification for {email}: {e}")
...
def lambda_handler(event, context):
    """Fetch usage from PromQL, update DynamoDB, check quotas, BLOCK + send alerts."""
    ...
    try:
        ...
        # Step 2: Read cumulative totals from DynamoDB
        current_month = now.strftime("%Y-%m")
        usage_data = {}
        ...
        # Step 4: Check sent alerts
        sent_alerts = get_sent_alerts(month_name)

        # Step 5: Check each user — ENFORCE BLOCKS + send alerts
        alerts_to_send = []
        stats = {"total_users": 0, "over_80": 0, "over_90": 0,
                 "exceeded": 0, "daily_exceeded": 0, "blocked": 0}
        ...
        for email, usage in usage_data.items():
            ...
            # ─── ENFORCEMENT: Block if exceeded ───
            if total_tokens > monthly_limit:
                stats["exceeded"] += 1
                # BLOCK the user
                block_user(email, policy, total_tokens)
                stats["blocked"] += 1
                # Send "Token Limit Reached" SNS (only once per month)
                block_alert_key = f"{email}#monthly#blocked"
                if block_alert_key not in sent_alerts:
                    send_block_notification(email, policy, total_tokens, month_name)
                    record_sent_alert(month_name, email, "monthly", "blocked", {
                        "current_usage": int(total_tokens), "limit": monthly_limit,
                    })
                    sent_alerts.add(block_alert_key)
            else:
                # Not over limit — unblock if previously blocked
                unblock_user(email)
                if monthly_pct > 90:
                    stats["over_90"] += 1
                elif monthly_pct > 80:
            ...
            # Generate warning/critical alerts (80%, 90%)
        ...
        print(f"Summary - Total: {stats['total_users']}, Over 80%: {stats['over_80']}, "
              f"Over 90%: {stats['over_90']}, Exceeded: {stats['exceeded']}, BLOCKED: {stats['blocked']}")
        ...
        for item in response.get("Items", []):
            ...
            if pt and ident:
                policies[f"{pt}:{ident}"] = {
                    ...
                    "enforcement_mode": item.get("enforcement_mode", "enforce"),
...
def resolve_user_quota(email, groups, policies_cache):
    ...
    if not ENABLE_FINEGRAINED_QUOTAS:
        return {
            ...
            "enforcement_mode": "enforce", "enabled": True,
        }
...
def check_limits_and_generate_alerts(email, total_tokens, daily_tokens, policy,
    ...
    """Check limits and generate alert dicts for 80% and 90% warnings."""
    alerts = []
    policy_info = f"{policy['policy_type']}:{policy['identifier']}"
    enforcement_mode = policy.get("enforcement_mode", "enforce")
    monthly_limit = policy["monthly_token_limit"]
    monthly_pct = (total_tokens / monthly_limit) * 100 if monthly_limit > 0 else 0
...
def send_alerts(alerts):
    """Send warning/critical alerts via SNS."""
    if not SNS_TOPIC_ARN:
        print("SNS_TOPIC_ARN not configured")
        return
    for alert in alerts:
        try:
            level_prefix = {"warning": "WARNING", "critical": "CRITICAL", "exceeded": "EXCEEDED"}.get(
                alert["alert_level"], "ALERT")
            type_label = {"monthly": "Monthly Token Quota", "daily": "Daily Token Quota"}.get(
                alert["alert_type"], "Quota")
            ...
            message = (
                ...
                f"Enforcement: {alert.get('enforcement_mode', 'enforce')}\n"
            )
            if alert["alert_level"] == "exceeded":
                message += "\nUSER HAS BEEN BLOCKED - model access denied until next billing period.\n"
            sns_client.publish(TopicArn=SNS_TOPIC_ARN, Subject=subject[:100], Message=message)
        except Exception as e:
